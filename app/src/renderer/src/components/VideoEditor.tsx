...
export const VideoEditor = ({ selectedVideoPath }: Props) => {
  ...
  const [allCategories, setAllCategories] = useState<string[]>(['Reflex', 'Positioning', 'Reactions']);
      ...
      <div className="mb-3">
        {selectedVideoPath ? (
          ...
          <div className="w-100 border border-secondary rounded bg-light text-center py-5">
            <p className="text-muted"><i className="bi bi-camera-reels"></i><br/>Select a video from the left panel.</p>
          </div>
        )}
      </div>

      {/* Timeline */}
      <div className="bg-white border rounded p-2 mb-5" style={{ height: '20px', position: 'relative' }}>
        {cutouts.map((cut, idx) => (
          <div
            key={idx}
            title={cut.label}
            ...
            style={{
              height: '100%',
              top: 0,
              left: `${(cut.start / (videoRef.current?.duration || 1)) * 100}%`,
              width: `${((cut.end - cut.start) / (videoRef.current?.duration || 1)) * 100}%`,
              opacity: 0.7,
            }}
          />
        ))}
      ...
      <form onSubmit={(e) => { e.preventDefault(); handleSaveCutout(); }}>
        <div className="row g-3 align-items-end">
          ...
          <div className="col-md-2">
            ...
            <input type="number" className="form-control" value={pre} min={0} onChange={(e) => setPre(+e.target.value)} />
          </div>
          
          <div className="col-md-2">
            <label className="form-label">Post (s):</label>
            <input type="number" className="form-control" value={post} min={0} onChange={(e) => setPost(+e.target.value)} />
          </div>
          <div className="col-md-5">
            ...
            <select className="form-select" value={zone} onChange={(e) => setZone(e.target.value)}>
              {Array.from({ length: 9 }, (_, i) => (
                <option key={i + 1} value={i + 1}>{i + 1} – area</option>
              ))}
            </select>
          </div>
