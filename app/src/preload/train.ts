...
const playerAPI = {
  ...
  onDelay: (cb: (seconds: number) => void) =>
    ipcRenderer.on(trainChannel.DELAY, (_evt, seconds) => cb(seconds)),

  offPlay: (cb) => ipcRenderer.removeListener(trainChannel.PLAY, cb),
  offPause: (cb) => ipcRenderer.removeListener(trainChannel.PAUSE, cb),
  offResume: (cb) => ipcRenderer.removeListener(trainChannel.RESUME, cb),
  offDelay: (cb) => ipcRenderer.removeListener(trainChannel.DELAY, cb),
